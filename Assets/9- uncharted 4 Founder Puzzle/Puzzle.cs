    ...
    public class Puzzle : MonoBehaviour
    {
        ...
        private bool CheckArrivingToDesiredPoint(int i)
        {
            ...
            if (points[i].switching)
            {
                ...
                if (Mathf.Abs(points[i].obj.transform.position.x - container.Splines[0].Knots.ToArray()[1].Position.x) <= 0.2f &&
                    Mathf.Abs(points[i].obj.transform.position.z - container.Splines[0].Knots.ToArray()[1].Position.z) <= 0.2f)
                {
                    ...
                    targetDistance[points[i].distanceIndex] = 0.1f + (0.1f * value);
                    value--;
